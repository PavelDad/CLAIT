...
{
    public partial class frmEquipmentFromEmployee : Form
    {
        ...
        public frmEquipmentFromEmployee()
        {
            this.Icon = frmMain.MyIcon;
            InitializeComponent();
        }

        private void pbxClear_MouseDown(object sender, MouseEventArgs e)
        {
            ((PictureBox)sender).Top += 2;
            ((PictureBox)sender).Left += 2;
        }

        private void pbxClear_MouseUp(object sender, MouseEventArgs e)
        {
            ((PictureBox)sender).Top -= 2;
            ((PictureBox)sender).Left -= 2;
        }

        private async void frmEquipmentFromEmployee_Load(object sender, EventArgs e)
        {
        ...
        private async void cbxEmployee_SelectedValueChanged(object sender, EventArgs e)
        {
            var employee = (Employee)cbxEmployee.SelectedItem;
            ...
            dgvResult.Columns[0].HeaderText = "Кабинет";
        ...
        private void pbxExcel_Click(object sender, EventArgs e)
        {
            ...
            workBook = excelApp.Workbooks.Open(Environment.CurrentDirectory + @"\Templates\EquipmentsFromEmployee.xlt");
            workSheet = (Microsoft.Office.Interop.Excel.Worksheet)workBook.Worksheets.get_Item(1);

            workSheet.Cells[2, 1] = $"Сотрудник: {((Employee)cbxEmployee.SelectedItem).Name} ({((Employee)cbxEmployee.SelectedItem).Position}), телефон: {((Employee)cbxEmployee.SelectedItem).Phone}";
            ...
            foreach (var item in sbl.ToList())
            {
                ...
                workSheet.Cells[i, 2] = (item.Cabinet != null) ? item.Cabinet.ToString() : ""; 
